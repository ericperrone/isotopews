...
import it.cnr.igg.isotopedb.queries.ItinerisSampleByReferenceDb;
import it.cnr.igg.itineris.NoKeyException;

@Path("")
public class ItinerisSamplesByReference extends ResultBuilder {
	@Context
	private HttpServletRequest request;

	@Path("/get-samples-by-reference")
	@OPTIONS
	...
	@Path("/get-samples-by-reference/{reference : (.+)?}")
	@GET
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response getSamples(@PathParam("reference") String reference) {
		Gson gson = new Gson();
		try {
			String key = Commons.getItinerisKeyFromHeader(request);
			ItinerisSampleByReferenceDb db = new ItinerisSampleByReferenceDb(key);
			ArrayList<Integer> list = db.getSamples(reference);
